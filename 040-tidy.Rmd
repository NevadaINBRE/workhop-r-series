...
### Lets try an example with biological data (16S)

**Messy**

```{r 040-boiled, echo=FALSE, eval=TRUE}
if (!file.exists("data/billboard.csv")) {
  download.file(
    # need to upload a microbiome OTU table to github and download that to here
    "https://raw.githubusercontent.com/hadley/tidy-data/master/data/billboard.csv",
    "data/billboard.csv"
  )
}
...
data %>% 
  ...
  rename(SampleName = Group) %>% 
  mutate(SampleName = metadata$SampleName) %>% 
  pivot_longer(-SampleName, names_to="otu", values_to = "count")

# then once the data is in a "long" format where each row is one observation, can merge with other DFs (like metadata) very easily
```
